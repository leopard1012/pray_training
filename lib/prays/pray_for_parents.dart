...
class _PrayForParents extends State<PrayForParents> {
  ...
  Future<List<TextSpan>> getPray() async {
    ...
    textSpanList.add(TextSpan(text: target, style: TextStyle(color: Colors.blue),
        recognizer: TapGestureRecognizer()..onTapDown = (p) => {Navigator.popAndPushNamed(context, '/conf/parents')} ));
    textSpanList.add(TextSpan(text: '를 통하여 거룩히 여김 받으시기를 원합니다.'));
    textSpanList.add(TextSpan(text: '그러므로 '));
    textSpanList.add(TextSpan(text: target, style: TextStyle(color: Colors.blue), recognizer: TapGestureRecognizer()..onTapDown = (p) => {Navigator.popAndPushNamed(context, '/conf/parents')} ));
    ...
    textSpanList.add(TextSpan(text: target, style: TextStyle(color: Colors.blue), recognizer: TapGestureRecognizer()..onTapDown = (p) => {Navigator.popAndPushNamed(context, '/conf/parents')} ));
    textSpanList.add(TextSpan(text: '가 하나님의 통치를 받으며 하나님의 백성답게 하나님 나라의 법을 지키며 살게 하옵시고, 하나님 나라의 영광을 위하여 일하게 하옵소서.'));
    textSpanList.add(TextSpan(text: '그리고 '));
    textSpanList.add(TextSpan(text: target, style: TextStyle(color: Colors.blue), recognizer: TapGestureRecognizer()..onTapDown = (p) => {Navigator.popAndPushNamed(context, '/conf/parents')} ));
    textSpanList.add(TextSpan(text: '를 통하여 하나님 나라가 세상 모든 사람들에게 전파되기를 원합니다.'));
    textSpanList.add(TextSpan(text: ''));
    ...
    textSpanList.add(TextSpan(text: target, style: TextStyle(color: Colors.blue), recognizer: TapGestureRecognizer()..onTapDown = (p) => {Navigator.popAndPushNamed(context, '/conf/parents')} ));
    textSpanList.add(TextSpan(text: '를 통하여 하나님의 선하신 뜻이 이루어지기를 기도합니다.'));
    ...
    textSpanList.add(TextSpan(text: ''));
    textSpanList.add(TextSpan(text: target, style: TextStyle(color: Colors.blue), recognizer: TapGestureRecognizer()..onTapDown = (p) => {Navigator.popAndPushNamed(context, '/conf/parents')} ));
    textSpanList.add(TextSpan(text: '가 세상의 욕심의 악에 물들지 않도록 지켜 주옵소서.'));
